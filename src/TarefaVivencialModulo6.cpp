...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (deltaT >= 1.0/FPS)
		{
			...
			background.iAnimation = (background.iAnimation + 1) % 100;
		}
		
		if (vampirao.iAnimation == 1){
			offsetTexBg.t = background.iAnimation * 0.01;
		}else if(vampirao.iAnimation == 2){
			offsetTexBg.t = background.iAnimation * - 0.01;
		}else if(vampirao.iAnimation == 3){
